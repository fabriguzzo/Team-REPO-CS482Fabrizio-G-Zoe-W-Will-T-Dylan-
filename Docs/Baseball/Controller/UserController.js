...
const Invitation = require('../Model/invitationDao.js');
...
exports.getOneUser = async function (req, res) {
  try {
    ...
    if (id) user = await dao.read(id);

    
    if (!user && username) user = await dao.readByUsername(username);
...
exports.getByRecipient = async function (req, res) {
  try {
    ...
    const parent = await dao.read(userId);
    if (!parent) {
      return res.status(404).json({ error: "Parent user not found" });
    }

    const allUsers = await dao.readAll();
...
exports.createOrUpdate = async function (req, res) {
  try {
    ...
    if (existingUser) {
      ...
      const updatedUser = await dao.updateByUsername(username, updatedData);
...
exports.deleteOne = async function (req, res) {
  ...
  try {
    const deleted = await dao.delete(username);
    if (deleted) {
  ...
  try {
    ...
    if (!childData.username || !childData.password || !childData.email) {
      return res.status(400).json({
        error: 'Missing required child fields: username, password, email'
      });
    }
